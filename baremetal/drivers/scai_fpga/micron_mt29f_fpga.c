...
static uint8_t get_feature(mt29f_register_t feature_addr) {
    const uint8_t cmd[2] = {MT29F_CMD_GET_FEATURES, (uint8_t)feature_addr};
    uint8_t result = 0;
...
uint8_t SCAI_MT29_Flash_read(uint8_t* buf, uint32_t addr, uint32_t len) {
    ...
    while (remaining_len > 0) {
        ...
        uint32_t row_addr = physical_addr >> 13;
        uint16_t col_addr = physical_addr & 0x1FFF;

        cmd[0] = MT29F_CMD_PAGE_READ_TO_CACHE;
        cmd[1] = (uint8_t)(row_addr >> 16);
        cmd[2] = (uint8_t)(row_addr >> 8);
        ...
        cmd[0] = (QSPI_FPGA_IF_get_io_format() == MSS_QSPI_QUAD_FULL) ? MT29F_CMD_READ_FROM_CACHE_X4 : MT29F_CMD_READ_FROM_CACHE_X1;
...
uint8_t SCAI_MT29_Flash_program(const uint8_t* buf, uint32_t addr, uint32_t len) {
    ...
    while (remaining_len > 0) {
        ...
        cmd[0] = (QSPI_FPGA_IF_get_io_format() == MSS_QSPI_QUAD_FULL) ? MT29F_CMD_PROGRAM_LOAD_X4 : MT29F_CMD_PROGRAM_LOAD_X1;
        cmd[1] = (uint8_t)(col_addr >> 8);
        cmd[2] = (uint8_t)(col_addr);
