...
void QSPI_FPGA_IF_transfer(const uint8_t* tx_buffer, uint32_t tx_len, uint8_t* rx_buffer, uint32_t rx_len, mss_qspi_io_format format, bool keep_ce_active) {
    ...
    ctrl1_val &= ~((0x7FFu << QSPI_CTRL1_RX_COUNT_SHIFT) | (0x7FFu << QSPI_CTRL1_TX_COUNT_SHIFT) | QSPI_CTRL1_QUAD_MODE | QSPI_CTRL1_BYTE_MODE);

    if (format == MSS_QSPI_QUAD_FULL) {
        ctrl1_val |= QSPI_CTRL1_QUAD_MODE;
    }

    ctrl1_val |= (tx_len << QSPI_CTRL1_TX_COUNT_SHIFT) | (rx_len << QSPI_CTRL1_RX_COUNT_SHIFT);
    ...
    *g_qspi_ctrl1_reg = ctrl1_val;
    ...
    ctrl1_val &= ~(QSPI_CTRL1_START_OP | (0x7FFu << QSPI_CTRL1_RX_COUNT_SHIFT) | (0x7FFu << QSPI_CTRL1_TX_COUNT_SHIFT));
    *g_qspi_ctrl1_reg = ctrl1_val;
}
