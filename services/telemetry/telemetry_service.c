...
static int32_t telemetry_ipi_status = 0;
static volatile uintptr_t __dma_addr = 0;
...
static void tm_monitoring_handler(struct StateMachine * const pMyMachine)
{
	(void)pMyMachine;
	char buf[4096];
	volatile uint8_t *__ddr_buf = NULL;

#if 1
	if (telemetry_ipi_status > 0) {
		format_log(HSS_HART_E51, buf, "telemetry_ipi_status : %d \r\n", telemetry_ipi_status);

		__ddr_buf = (volatile uint8_t *)__dma_addr;
		format_log(HSS_HART_E51, buf, "__ddr_buf[4] : 0x%02X \r\n", __ddr_buf[4]);
		format_log(HSS_HART_E51, buf, "__ddr_buf[5] : 0x%02X \r\n", __ddr_buf[5]);
		format_log(HSS_HART_E51, buf, "__ddr_buf[6] : 0x%02X \r\n", __ddr_buf[6]);
		format_log(HSS_HART_E51, buf, "__ddr_buf[7] : 0x%02X \r\n", __ddr_buf[7]);
		__ddr_buf[4] = 0x04;
		__ddr_buf[5] = 0x05;
		__ddr_buf[6] = 0x06;
		__ddr_buf[7] = 0x07;
		...
		memcpy((void *)__ddr_buf, buf, strlen(buf) + 1);
		format_log(HSS_HART_E51, buf, "strlen(buf) : %d \r\n", strlen(buf));
		//format_log(HSS_HART_E51, buf, NULL);
		//memset(buf, 0, sizeof(buf));
		__dma_addr = 0;

		telemetry_ipi_status = (telemetry_ipi_status == 1) ? 0 : telemetry_ipi_status - 1;
	}
#else
    ...
    uint32_t immediate_arg, void *p_extended_buffer_in_ddr, void *p_ancilliary_buffer_in_ddr)
{
    ...
    //custom_uart_printf(HSS_HART_E51, "%s!!!\r\n", __func__);
		//tm_monitoring_print();
